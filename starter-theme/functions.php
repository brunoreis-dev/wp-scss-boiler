...
/**
 * Function to include component dynamically with props.
 */
function include_component($file_name, $variables = array()) {
  $output = NULL;
  $file_path = 'components/' . $file_name . '/' . $file_name . '.php';
  $file = locate_template($file_path);
  if(!empty($file)) {
    extract($variables);
    ob_start();
    include $file_path;
    ...
    print '<p>Component <strong>' . $file_name . '</strong> not found.</p>';
  }
}
